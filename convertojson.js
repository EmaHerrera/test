module.exports = /******/ (function (modules, runtime) {
  ...
  /******/ {
    /***/ 568: /***/ function (
      ...
      const ST = __webpack_require__(998);
      var fs = __webpack_require__(747);
      var contents = fs.readFileSync(process.argv[2]);
      var data = JSON.parse(contents);

      var child_modules_template = {
        "{{#each planned_values.root_module.child_modules}}": {
          index: "{{$index}}",
        },
      };
      var modules = ST.transform(child_modules_template, data);
      var module_json_out = {};
      var output = {};
      modules.forEach((element, index) => {
        var e =
          ...
          "].resources}}";
        var e1 =
          "{{planned_values.root_module.child_modules[" + index + "].address}}";
        var values_template = {
          [e]: {
            "{{address}}": "{{values}}",
            name: "{{address}}",
        ...
        var values = ST.transform(values_template, data);
        var values_string = JSON.stringify(values, null, "  ")
          .replace(/module./g, "")
          .replace(/\[(\d)\]/g, ".$1");
        var values_array = JSON.parse(values_string);

        var module_template = {
          module_name: e1,
        };

        var module = ST.transform(module_template, data);
        var module_name = module.module_name.replace(/module./g, "");
        var replace_exp = module_name + "\\.";
        var re = new RegExp(replace_exp, "g");

        var module_string = JSON.stringify(module, null, "  ")
          .replace(/module./g, "")
          .replace(/\[(\d)\]/g, ".$1");
        var module_json = JSON.parse(module_string);
        module_json_out[module.module_name] = {
          destroy: false,
        };
        for (let val of values_array) {
          var v = val[val.name];
          Object.keys(v).forEach(function (key) {
            ...
            if (Array.isArray(value)) {
              if (value.length > 0) {
                v[key + ".#"] = "" + value.length;
                value.forEach(function (vo, index) {
              ...
              var vk = Object.keys(value);
              if (vk.length > 0) {
                v[key + ".%"] = "" + vk.length;

                vk.forEach(function (vk_k, i) {
                  v[key + "." + vk_k] = "" + value[vk_k];
                });
              }
            }
          });
          module_json_out[module.module_name][val.name] = val[val.name];
        }
        output = JSON.stringify(module_json_out, null, "  ")
          .replace(/module./g, "")
          .replace(re, "");
      });
      console.log(output);
        ...
        var Conditional = {
          ...
          is: function (template) {
            ...
            for (var last_key in last) {
              ...
              if (
                ["#else", "#elseif"].indexOf(func.name.toLowerCase()) === -1
              ) {
                return false;
              }
            }
        ...
        var TRANSFORM = {
          transform: function (template, data, injection, serialized) {
            ...
            if (injection) {
              ...
              var resolved_template = SELECT.select(
                template,
                selector,
                serialized
              )
            ...
            try {
              ...
              if (data) {
                ...
                if (match) {
                  ...
                  func = Function("with(this) {return (" + slot + ")}").bind(
                    data
                  );
                }
                var evaluated = func();
      ...
      */
          ...
          transformWith: function (obj, serialized) {
            ...
            if (SELECT.$selected && SELECT.$selected.length > 0) {
              ...
              var parsed_object = TRANSFORM.run(
                template,
                SELECT.$selected_root
              );
              // apply the result to root
